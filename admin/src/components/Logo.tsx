...
const Logo: React.FC<LogoProps> = ({ className = '', showText = true, size = 'md' }) => {
  ...
  return (
    <div className={`flex items-center space-x-3 ${className}`}>
      {/* Premium Admin RWU Logo */}
      <div className={`${sizeClasses[size]} relative group`}>
        <div className="w-full h-full bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 rounded-2xl flex items-center justify-center shadow-2xl border-2 border-blue-400/30 group-hover:scale-105 transition-transform duration-300 overflow-hidden relative">
          {/* Premium Background Pattern */}
          <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent"></div>

          {/* Admin Crown Effect */}
          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-4 h-2 bg-gradient-to-b from-blue-300 to-transparent rounded-b-sm opacity-60"></div>
